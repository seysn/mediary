...
use crate::{error::EbmlResult, vint::Vint};
...
macro_rules! declare_elements {
    ($($k:ident($id:literal, $ty:expr)),+) => {
        ...
        impl From<EbmlId> for EbmlElement {
            fn from(value: EbmlId) -> Self {
                match value {
                    $(EbmlId($id) => Self::$k,)+
                    _ => EbmlElement::Custom(value),
                }
            }
        }
        ...
        impl EbmlElement {
            ...
            pub fn kind(&self) -> ElementType {
                match self {
                    $(Self::$k => $ty,)+
                    Self::Custom(_) => ElementType::Binary,
                }
            }
        }
    };
...
impl EbmlElement {
    pub fn from_reader<R: Read>(reader: &mut R) -> EbmlResult<Self> {
        Ok(Self::from(EbmlId::from_reader(reader)?))
    }
}
...
declare_elements!(
    ...
    Ebml(0x1A45DFA3, ElementType::Master),
    EbmlVersion(0x4286, ElementType::UnsignedInteger),
    EbmlReadVersion(0x42F7, ElementType::UnsignedInteger),
    EbmlMaxIDLength(0x42F2, ElementType::UnsignedInteger),
    EbmlMaxSizeLength(0x42F3, ElementType::UnsignedInteger),
    DocType(0x4282, ElementType::String),
    DocTypeVersion(0x4287, ElementType::UnsignedInteger),
    DocTypeReadVersion(0x4285, ElementType::UnsignedInteger),
    DocTypeExtension(0x4281, ElementType::Master),
    DocTypeExtensionName(0x4283, ElementType::String),
    DocTypeExtensionVersion(0x4284, ElementType::UnsignedInteger),
    Crc32(0xBF, ElementType::Binary),
    Void(0xEC, ElementType::Binary),
    // Matroska Schema
